...
![图 7-1 身份与组关系](../assets/diagrams/users.svg)
...
getent 可理解为 get entries，按系统配置查询名称服务数据库条目。passwd 在这里是数据库名，不是在改密码。$(id -un) 先查询当前有效用户名，再把结果作为一个参数交给 getent。-u 查询有效用户 ID，-n 改为显示名字。环境变量 USER 可能因切换身份或人为赋值而保留旧值，查询当前身份时优先使用 id。passwd 条目通常包含用户名、密码占位符、UID、GID、说明、家目录和登录 Shell，字段以冒号分隔。密码散列通常放在受保护的 /etc/shadow 中，不应复制到学习笔记。
...
> 实机记录｜本次目标机启用了 wheel 的 sudo 策略，普通测试账户未获授权时被拒绝；管理步骤由已有 root 连接执行。没有向测试账户授予免密 sudo。读者以带密码的普通账户完成 sudo 授权与交互登录，仍需按自己的安装策略验证。
